...
def run_sender_basic(filename):
    try:
        subprocess.run(["python", "file-sender.py", filename])
    except Exception as e:
        print(f"Error executing sender-basic.py: {e}")

# Function to handle communication with the client
def server_thread():
    Vilib.camera_start(vflip=False,hflip=False)
    Vilib.display(local=False,web=True)
    path =f"{user_home}/picar-x/tcp"
    while True:
        global speedd
        ...
        if command == "nota":
            ...
            full_filepath = ("%s.jpg"%(_time))
            sender_thread = threading.Thread(target=run_sender_basic, args=(full_filepath,))
            sender_thread.start()
            #time.sleep(1)
